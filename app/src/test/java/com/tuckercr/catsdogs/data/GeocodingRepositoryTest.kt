...
class GeocodingRepositoryTest {
    ...
    @Test
    fun `searchCities trims query and maps city labels`() = runBlocking {
    ...
    @Test
    fun `searchCities returns empty result for short trimmed queries without calling api`() = runBlocking {
        val api = RecordingGeocodingApi()
        val repository = GeocodingRepository(api, "test-key")
    ...
    @Test
    fun `searchCities fails before api call when api key is missing`() = runBlocking {
        val api = RecordingGeocodingApi()
