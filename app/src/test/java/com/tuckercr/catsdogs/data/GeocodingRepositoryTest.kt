...
class GeocodingRepositoryTest {
    ...
    @Test
    fun `short query returns empty suggestions without calling api`() = runBlocking {
    ...
    @Test
    fun `search trims query and maps city labels`() = runBlocking {
        val api = FakeGeocodingApi(
            results = listOf(
                GeocodingDirectDto(
                    name = "London",
    ...
    @Test
    fun `missing api key fails without calling api`() = runBlocking {
        val api = FakeGeocodingApi()
