...
class GeocodingRepositoryTest {
    ...
    @Test
    fun `searchCities rejects blank api key before calling api`() =
        runBlocking {
    ...
    @Test
    fun `searchCities ignores short trimmed queries without calling api`() =
        runBlocking {
            val api = FakeGeocodingApi()
    ...
    @Test
    fun `searchCities trims query and maps labels with optional state`() =
        runBlocking {
            val api = FakeGeocodingApi(
                response = listOf(
                    GeocodingDirectDto(
