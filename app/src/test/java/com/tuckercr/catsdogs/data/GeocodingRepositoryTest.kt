...
class GeocodingRepositoryTest {
    ...
    @Test
    fun `short queries return empty suggestions without calling API`() = runBlocking {
    ...
    @Test
    fun `missing API key fails before calling API`() = runBlocking {
        val api = FakeGeocodingApi()
    ...
    @Test
    fun `search trims query and maps labels without blank state`() = runBlocking {
        val api = FakeGeocodingApi(
            responses = listOf(
